defmodule Arrow.Ipc.Flatbuf.Interval do
  # Generated from FlatBuffers table Arrow.Ipc.Flatbuf.Interval. Do not edit.
  @moduledoc false
    ...
    Map.new([
      {"unit",
       if(Map.get(value, :unit) == nil,
         do: nil,
         else: Arrow.Ipc.Flatbuf.IntervalUnit.__to_json__(Map.get(value, :unit))
       )}
    ])
    ...
    %__MODULE__{
      unit:
        if(Map.get(map, "unit") == nil,
          do: nil,
          else: Arrow.Ipc.Flatbuf.IntervalUnit.__from_json__(Map.get(map, "unit"))
        )
    }
  end
