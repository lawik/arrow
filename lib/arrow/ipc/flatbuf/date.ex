defmodule Arrow.Ipc.Flatbuf.Date do
  # Generated from FlatBuffers table Arrow.Ipc.Flatbuf.Date. Do not edit.
  @moduledoc false
    ...
    Map.new([
      {"unit",
       if(Map.get(value, :unit) == nil,
         do: nil,
         else: Arrow.Ipc.Flatbuf.DateUnit.__to_json__(Map.get(value, :unit))
       )}
    ])
    ...
    %__MODULE__{
      unit:
        if(Map.get(map, "unit") == nil,
          do: nil,
          else: Arrow.Ipc.Flatbuf.DateUnit.__from_json__(Map.get(map, "unit"))
        )
    }
  end
