defmodule Arrow.Ipc.Flatbuf.BodyCompressionMethod do
  ...
  def __from_json__(name) when is_binary(name) do
    ...
    if atom in all(),
      do: atom,
      else: raise("unknown Arrow.Ipc.Flatbuf.BodyCompressionMethod variant: " <> name)
  end

  def __from_json__(int) when is_integer(int) do
    from_value(int) ||
      raise("unknown Arrow.Ipc.Flatbuf.BodyCompressionMethod value: " <> Integer.to_string(int))
  end
end
